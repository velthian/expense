...
if($statement_id !== null)
{
    ...
    catch (Throwable $e) 
    {
        ...
        // Friendly message to UI / logs
        $reconArray = [
            'status' => false,
            'message' => 'Reconciliation failed. Please check logs.',
            'error' => $e->getMessage(), // optional: hide in prod
        ];
    }
    ...
    if(!empty($reconArray))
    {
        ...
        $reconciledAmount = 0;
        foreach($csvReconciledArray as $rec)
        {
            $reconciledAmount += $rec['amount'];
        }

        $unReconciledAmount = 0;
        foreach($csvUnReconciledArray as $unrec)
        {
            $unReconciledAmount += $unrec['amount'];
        }

        $netOutGoByStatement = $reconciledAmount + $unReconciledAmount;

        if($netOutGoByStatement === $netOutGo)
        {
            $reconciledFlag = TRUE;
        }
    ...
    <div id="bankReconTitle">BANK ACCOUNT RECONCILIATION</div>
    <div id="statementDates"><?php echo($beginDate . " - " . $lastDate);?></div>
    <div id ="bankReconCont">
        <div class="bankReconRow">
            <div class="bankReconRowHeader">Opening Balance</div>
            <div class="bankReconRowValues"><?php echo($currencyFormatObj->convertToIndianCurrency($openingBalance)); ?></div>
        </div>
        <div class="bankReconRow">
            <div class="bankReconRowHeader">Closing Balance</div>
            <div class="bankReconRowValues"><?php echo($currencyFormatObj->convertToIndianCurrency($closingBalance)); ?></div>
        </div>
        <div class="bankReconRow">
            <div class="bankReconRowHeader">Net Expenditure</div>
            <div class="bankReconRowValues"><?php echo($currencyFormatObj->convertToIndianCurrency($netOutGo)); ?></div>
        </div>
        <div class="bankReconRow">
            <div class="bankReconRowHeader">Sum of Expense By Transaction</div>
            <div class="bankReconRowValues"><?php echo($currencyFormatObj->convertToIndianCurrency($netOutGoByStatement)); ?></div>
        </div>
        <div id="statementReconFlag"
            style=" background: <?= $reconciledFlag ? '#bcf5bc' : '#ffb09c' ?>;
                    color: <?= $reconciledFlag ? 'green' : '#ee2400' ?>;
                    padding: 5px;
                    border-radius: 5px;
                    font-size: 14px;
                    margin-top: 10px;
...
$('body').on('click','.csvUpdateBtn', function(){
    ...
    $.ajax({
        ...
        method: 'POST',
        dataType: 'json',
        data: { line_id: lineId, uid: uidValue, calledFrom: "bank" } // uid is mandatory per your latest requirement
        })
        .done(function (res) 
        {
